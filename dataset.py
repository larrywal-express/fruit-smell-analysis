# python dataset.py --raw_dataset_path raw/fruits
import argparse
import json
...
def create_dataset(args):
    ...
    #fix classes
    df['class_name'] = df['class_label'].astype('category')
    ...
    for i, row in df.iterrows():
        if row['class_label'] not in class_list:
            class_list[row['class_label']] = 0
        class_list[row['class_label']] += 1
        if class_list[row['class_label']] <= args.demo_max_records:
...
def process_timeseries_train_test(args, raw_dfs, min_sample_size, train=True):
    ...
    df = df.reset_index()
    df.rename(columns={'index': 'timepoints'}, inplace=True)
    df.rename(columns={'class_label': 'class_val'}, inplace=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--raw_dataset_path', type=str, default='raw/fruits', help='location of dataset')
